#Video https://youtu.be/LzMnZDzK1fQ
...
# print("validaiton accuracy -- ",val_accuracy)
...
# Fitting the ANN to the Training set
start = time.time()
history = multi_dnn.fit(X_train, y_train,epochs=10, validation_data=(X_valid,y_valid),batch_size=50,verbose=2)
end_train = time.time()
...
start = time.time()
history = multi_cnn.fit(X_train, y_train, epochs=10,validation_data=(X_valid,y_valid), batch_size=50)
end_train = time.time()
y_pred = np.argmax(multi_cnn.predict(X_test), axis=1)
end_predict = time.time()
val_accuracy = max(history.history['val_accuracy'])
result(y_pred,y_test,"CNN",start,end_train,end_predict,val_accuracy)
...
start = time.time()
history  = rnn_multi.fit(X_train, y_train,validation_data=(X_valid,y_valid), epochs=10, batch_size=50)
end_train = time.time()
...
end_predict = time.time()
y_pred_valid = elm_classifier.predict(X_valid)
val_accuracy = accuracy_score(y_valid,y_pred_valid)
result(y_pred,y_test,"ELM",start,end_train,end_predict,val_accuracy)
...
start = time.time()
history = dann_model.fit(X_train, {'source_classifier': y_train_categorical, 'domain_classifier': np.zeros((len(X_train), 1))},
...
# Train the model
start = time.time()
history = dbm_model.fit(X_train, y_train, epochs=10, batch_size=50,validation_data=(X_valid,y_valid), shuffle=True)
end_train = time.time()
## Predict on the test set
...
autoencoder.fit(X_train, X_train, epochs=10, batch_size=50, shuffle=True, validation_data=(X_valid, X_valid))
...
y_pred = classifier.predict(X_test)
end_predict=time.time()
y_val_pred = lda.predict(X_valid)
val_accuracy = accuracy_score(y_valid, y_val_pred)
...
# **MULTI-CLASS CLASSIFICATION**
## **Data Splitting**
# reloading many transformations on X,Y causing errors for previous gru code
X_train = train_data.drop(columns=['label'],axis=1)
X_test = test_data.drop(columns=['label'],axis=1)
y_train = train_data['label']
y_test = test_data['label']
...
start = time.time()
history = rnn_multi.fit(X_train, y_train, epochs=10, batch_size=50)
end_train = time.time()
y_pred = np.argmax(rnn_multi.predict(X_test), axis=1)
end_predict = time.time()
...
end_predict = time.time()
y_val_pred = svm_multi.predict(X_valid)

val_accuracy = accuracy_score(y_valid, y_val_pred)
result(y_pred,y_test,"SVM",start,end_train,end_predict,val_accuracy)
